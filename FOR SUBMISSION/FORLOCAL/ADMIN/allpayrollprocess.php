							...
							if (mysqli_num_rows($loanexecqry) > 0) {
								...
								while ($loanrow = mysqli_fetch_assoc($loanexecqry)) {
									...
									$noofpays = $noofpays-1;
										...
										$loanhistory="INSERT INTO loan_history (uniquekey, loan_id, loantype, loanorg, emp_id, lastname, firstname, middlename,  start_date, end_date, monthly_payment, status, num_of_payments, payperiod, admin_name) VALUES
										('$uniquekey','$loanID','$loantype','$loanorg','$payID','$lname','$fname','$mname','$startdate','$enddate','$monthly_deduct', '$loan_status','$noofpays','$payperiodrange','$adminFullName')";
			...
			$secondhalf = $totalnet - $firsthalf;
						...
						if ($searchrows >=1){
							...
							<script>
								...
								$savepayrollquery = "INSERT INTO pay_per_period (emp_id,pperiod_range,pperiod_month,pperiod_year,rate_per_hour,hours_worked, reg_pay, undertimehours, absences,net_pay, firsthalf, secondhalf) VALUES 
																					('$payID','$payperiodrange','$month','$year','$rph','$hw','$salary', '$totalut', '$totalabsences', '$totalnet', '$firsthalf', '$secondhalf')";
								$savepayrollexecquery = mysqli_query($conn,$savepayrollquery) or die ("FAILED TO INSERT PAYROLL INFO ".mysqli_error($conn));
