...
if (isset($_GET['print_btn'])) {
  ...
  $searchquery .= "ORDER BY CASE WHEN leave_status = 'Pending' THEN 1 ELSE 2 END, $sortColumn $sortOrder LIMIT $start_from,".$results_perpage;
...
if (empty($search_result)) {
  ...
  JOIN employmenttypes ON employmenttypes.employment_TYPE = employees.employment_TYPE
  JOIN leaves_application ON employees.emp_id = leaves_application.emp_id
  JOIN position ON position.position_name = employees.position ORDER BY CASE WHEN leave_status = 'Pending' THEN 1 ELSE 2 END, $sortColumn $sortOrder";
  $start_from = ($page - 1) * $results_perpage;
